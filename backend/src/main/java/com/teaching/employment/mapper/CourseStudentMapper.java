...
public interface CourseStudentMapper extends BaseMapper<CourseStudent> {
    ...
    @Select("SELECT cs.*, c.course_name " +
            ...
            "WHERE cs.student_id = #{studentId} AND cs.is_deleted = 0")
    List<CourseStudent> selectCoursesByStudentId(@Param("studentId") Long studentId);

    /**
     * 统计课程学员数量
     */
    @Select("SELECT COUNT(*) FROM t_course_student WHERE course_id = #{courseId} AND is_deleted = 0")
    Integer countStudentsByCourseId(@Param("courseId") Long courseId);

    /**
     * 检查学员是否已选课
     */
    @Select("SELECT COUNT(*) FROM t_course_student WHERE course_id = #{courseId} AND student_id = #{studentId} AND is_deleted = 0")
