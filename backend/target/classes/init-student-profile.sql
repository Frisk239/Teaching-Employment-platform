...
-- 3. 扩展用户表 - 添加学号和简历URL字段（如果不存在）
-- 使用存储过程安全地添加字段
DELIMITER $$
CREATE PROCEDURE AddUserColumnsIfNotExists()
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE t_user ADD COLUMN student_number VARCHAR(50) COMMENT '学号';
  END IF;
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE t_user ADD COLUMN resume_url VARCHAR(255) COMMENT '简历文件URL';
  END IF;
END$$
DELIMITER ;

CALL AddUserColumnsIfNotExists();
DROP PROCEDURE AddUserColumnsIfNotExists;

-- 4. 扩展课程表 - 添加周次字段（如果不存在）
DELIMITER $$
CREATE PROCEDURE AddTimetableColumnIfNotExists()
BEGIN
  IF NOT EXISTS (
    SELECT * FROM information_schema.columns
    WHERE table_schema = DATABASE()
    AND table_name = 't_timetable'
    AND column_name = 'week_number'
  ) THEN
    ALTER TABLE t_timetable ADD COLUMN week_number INT DEFAULT 1 COMMENT '周次';
  END IF;
END$$
DELIMITER ;

CALL AddTimetableColumnIfNotExists();
DROP PROCEDURE AddTimetableColumnIfNotExists;
...
-- 8. 添加索引以优化查询性能
CREATE INDEX IF NOT EXISTS idx_education_user_dates ON t_education(user_id, start_date DESC);
CREATE INDEX IF NOT EXISTS idx_internship_user_dates ON t_internship(user_id, start_date DESC);
