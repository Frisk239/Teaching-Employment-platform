...
-- 删除求职申请记录
DELETE FROM t_job_application WHERE id IN (
...
DELETE FROM t_student WHERE id NOT IN (
    SELECT DISTINCT user_id FROM t_user WHERE id BETWEEN 10 AND 17
);
...
SELECT
    ...
    '用户表记录数:' AS description,
    COUNT(*) AS count
FROM t_user WHERE id BETWEEN 10 AND 17

UNION ALL

SELECT
    '学生表记录数:' AS description,
    COUNT(*) AS count
FROM t_student WHERE id BETWEEN 1 AND 8

UNION ALL

SELECT
    '学生表有userId的记录数:' AS description,
    COUNT(*) AS count
FROM t_student WHERE id BETWEEN 1 AND 8 AND user_id IS NOT NULL

UNION ALL

SELECT
    '简历表记录数:' AS description,
    COUNT(*) AS count
FROM t_resume WHERE id BETWEEN 1 AND 8

UNION ALL

SELECT
    '求职申请表记录数:' AS description,
    COUNT(*) AS count
